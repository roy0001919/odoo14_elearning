...
odoo.define('web_widget_x2many_excel.widget', function (require) {
    "use strict";
    ...
    var $ = require('jquery');
    $.getScript( "/web_widget_x2many_excel/static/lib/ag-grid.min.js" )

    var WidgetX2ManyExcel = FieldOne2Many.extend({
        template: 'FieldX2ManyExcel',
        widget_class: 'web_widget_x2many_excel',
        ...
        get_field_value: function(row, field, many2one_as_name)
        {
           // console.log(row);
           // console.log(field);
           // console.log(this.fields[field].type);
           // console.log(many2one_as_name);
        ...
        {
            // console.log("row: " + row);
            // console.log("this.field_x_axis: " + this.field_x_axis);
            // console.log("this.get_field_value: " + this.get_field_value(row, 'net_weight'));
            if(this.x.slice(-1) == '*')this.x=this.x.slice(0,-1);
            var x = this.get_field_value(row, this.x),
        ...
        compute_totals: function()
        {
            ...
            _.each(rows, function(row)
            {
                var key_x = self.get_field_value(row, self.x),
        ...
        excel_grid: function() {
            var self = this;
            var x_axis_values = self.get_x_axis_values();
            var y_axis_values = self.get_y_axis_values();
            // for (var i in self.fields){
            //     console.log(i);
            //     console.log(self.fields[i].string);
            // }
            var base_fields = ['product_id', 'name', 'part_no', 'supplier_part_no', 'material', 'net_weight', 'length'];

            var columnDefs = [];
            for (var f=0;f<base_fields.length;f++){
                if (self.fields[base_fields[f]]){
                    columnDefs.push({headerName: self.fields[base_fields[f]].string, field: base_fields[f]});
                }
            }
            for (var x=0;x < x_axis_values.length; x++){
                columnDefs.push({headerName: self.get_x_axis_label(x_axis_values[x]), field: x_axis_values[x]});
                // console.log(self.get_x_axis_label(x_axis_values[x]));
                // for (var y=0;y < y_axis_values.length; y++){
                //     console.log(self.get_xy_id(x_axis_values[x], y_axis_values[y]));
                //     // console.log(self.get_y_axis_label(y_axis_values[y]));
                // }
            }
            var rowData = [];
            for (var y=0;y < y_axis_values.length; y++){

                // var row = {test: self.get_y_axis_label('name')};
                // console.log(self.get_field_value(_.first(_.values(y_axis_values[y])),this.field_label_y_axis, true));
                var row = {product: self.get_y_axis_label(y_axis_values[y])};
                  // return this.get_field_value(
                  //           _.first(_.values(this.by_y_axis[y])),
                  //           this.field_label_y_axis, true);
                // console.log(_.values(self.by_y_axis[y]));
                // console.log(_.first(_.values(self.by_y_axis[y])), self.field_label_y_axis, true);
                for (var x=0;x < x_axis_values.length; x++){
                    row[x_axis_values[x]] = self.format_xy_value(self.get_xy_value(x_axis_values[x], y_axis_values[y]));
                }
                rowData.push(row);
                // console.log(self.get_xy_id(x_axis_values[x], y_axis_values[y]));
                // console.log(self.get_y_axis_label(y_axis_values[y]));
            ...
            var eGridDiv = self.$el.find('#excelGrid')[0];
            new agGrid.Grid(eGridDiv, gridOptions);
        }

    });

    core.form_widget_registry.add('x2many_excel', WidgetX2ManyExcel);

    return WidgetX2ManyExcel;
});
