...
class LioulibridgeReport(http.Controller):

	search_fields = ['order_num', 'product_entry', 'product_cate', 'batch_info', 'order_user', 'buyer_tel', 'product_uom_qty','contact_usr', 'contact_tel' , 'checkin_time']
	heaser_fields = [u'預訂編號', u'入口', u'票種', u'日期梯次', u'預訂人', u'預訂人聯絡電話', u'人數', u'參訪人', u'參訪人聯絡電話', u'入場時間']

	@http.route('/lioulibridge/report', type='http', auth='public')
	@serialize_exception
	def download_bin(self, d, filename=None, **kw):
		sol = request.env['sale.order.line']
		# _logger.info('input d :%s' % d)
		res = sol.sudo().search_read([('remark1', 'not in', [u'上線預扣']), ('state', 'in', ['sale', 'done']), ('p_date', '=', d), ('note', '=', False)], self.search_fields, order='booking_start')
